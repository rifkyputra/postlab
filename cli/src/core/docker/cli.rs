...
use crate::core::{
    docker::{ContainerInspect, DockerManager},
...
impl DockerManager for DockerCliManager {
    ...
    async fn inspect_container(&self, container: &str) -> Result<ContainerInspect> {
        ...
        // Ports: NetworkSettings.Ports → "8080/tcp" → ["0.0.0.0:8080->8080/tcp"]
                ...
                ports_map
                    ...
                    .filter_map(|(container_port, bindings)| {
                        let host_ports: Vec<String> = bindings
                            ...
                            .iter()
                            .filter_map(|b| {
                                let host_ip = b["HostIp"].as_str().unwrap_or("0.0.0.0");
        ...
        // Volumes: Mounts → [{"Source": "/host", "Destination": "/container"}]
        let volumes = info["Mounts"]
            .as_array()
            .unwrap_or(&vec![])
            .iter()
            .filter_map(|m| {
        ...
        // Env vars: Config.Env → ["KEY=value", ...]
        let env_vars = info["Config"]["Env"]
            .as_array()
            .unwrap_or(&vec![])
            .iter()
            .filter_map(|e| {
        ...
        // Docker health: State.Health.Status
        let docker_health = info["State"]["Health"]["Status"]
            .as_str()
            .unwrap_or("")
            .to_string();
    ...
    async fn run_named_container(
        ...
        let restart_arg = format!("--restart={}", restart);
        args.push(&restart_arg);
        args.push(image);
        let out = Command::new(self.bin).args(&args).output().await?;
        if out.status.success() {
            Ok(())

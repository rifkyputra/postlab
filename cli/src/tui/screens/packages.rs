...
fn render_search(f: &mut Frame, app: &App, area: Rect) {
    ...
    let cursor = if app.packages.search_mode == InputMode::Editing {
        "█"
    } else {
        ""
    };
...
fn render_quick_install(f: &mut Frame, app: &App, area: Rect) {
    ...
    let installed_names: std::collections::HashSet<&str> = app
        .packages
        .installed
        .iter()
        .map(|p| p.name.as_str())
...
fn render_queue(f: &mut Frame, app: &App, area: Rect) {
    ...
    let (title, output_text, title_color) = if let Some(sel) = app.packages.queue_selected {
        ...
        (
            " Output — select an item with ↑/↓ ".to_string(),
            String::new(),
            Color::DarkGray,
        )
    };
    ...
    let pane = Paragraph::new(display_lines).block(
        Block::default()
